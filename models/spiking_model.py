...
#     def __init__(self, layers=(784, 400, 10), decay=0.2):
#         super().__init__()
...
thresh = 0.5 # neuronal threshold
lens = 0.5 # hyper-parameters of approximate function
decay = 0.2 # decay constants (global default)
num_classes = 10
batch_size  = 100
learning_rate = 1e-3
num_epochs = 100 # max epoch
...
class SMLP_Fast(nn.Module):
    """
    Fast batch-processing SMLP with instance-level decay.
    
    ~100x faster than Event_SMLP_Quantized!
    
    Use for:
    - Training experiments (decay investigation, architecture search, etc.)
    - When you DON'T need event-driven processing
    - When you DON'T need quantization
    
    Performance comparison (CPU, 100 samples):
    - SMLP_Fast: ~0.2-0.5 seconds per batch
    - Event_SMLP_Quantized: ~20-40 seconds per batch
    
    NOTE: Has different decay dynamics than Event_SMLP!
    Use Event_SMLP_Fast if you need correct event-driven dynamics.
    """
    ...
    def forward(self, input, time_window=20):
        """
        Fast batch processing - processes all samples in parallel.
        ...
        Returns:
            outputs: [B, num_classes] - rate-coded outputs
        """
        B = input.size(0)
        num_layers = len(self.layers)
        
        # Initialize membrane/spike states for each layer (batched)
        mem = [torch.zeros(B, layer.out_features, device=input.device) 
               for layer in self.layers]
        spike = [torch.zeros(B, layer.out_features, device=input.device) 
                 for layer in self.layers]
        sumspike = [torch.zeros(B, layer.out_features, device=input.device) 
                    for layer in self.layers]
        ...
        for t in range(time_window):
            ...
            # Process through all layers (vectorized across batch)
            for i, layer in enumerate(self.layers):
                # Membrane update with instance decay
                mem[i] = mem[i] * self.decay * (1. - spike[i]) + layer(x)
                spike[i] = act_fun(mem[i])
                sumspike[i] += spike[i]
                x = spike[i]  # Output of this layer is input to next
...
class Event_SMLP_Fast(nn.Module):
    """
    Fast batch-processing Event-SMLP with correct event-driven decay dynamics.
    
    Key difference from SMLP_Fast: Decay order matches Event_SMLP
    - SMLP_Fast: Decay → Accumulate → Spike
    - Event_SMLP_Fast: Accumulate → Spike → Reset → Decay
    
    This makes Event_SMLP_Fast MORE SENSITIVE to decay parameter changes,
    which is important for investigating decay effects!
    
    Performance (CPU, 100 samples):
    - Event_SMLP_Fast: ~0.3-0.8 seconds per batch
    - Event_SMLP: ~20-40 seconds per batch
    - SMLP_Fast: ~0.2-0.5 seconds (but different dynamics!)
    
    Use for:
    - Investigating decay in event-driven SNNs
    - When you need correct event-driven temporal dynamics
    - Training with 10-50x speedup vs sample-by-sample Event_SMLP
    """
    ...
    def forward(self, input, time_window=20):
        """
        Event-driven batch processing with correct decay order.
        
        Order: 1. Accumulate → 2. Spike → 3. Reset → 4. Decay
        ...
        for t in range(time_window):
            ...
            # Process through all layers with EVENT-DRIVEN order
            for i, layer in enumerate(self.layers):
                # 1. ACCUMULATE: Add weighted inputs to membrane
                mem[i] = mem[i] + layer(x)
                
                # 2. SPIKE: Generate spikes based on membrane potential
                spike[i] = act_fun(mem[i])
                sumspike[i] += spike[i]
                
                # 3. RESET: Set spiking neurons to 0
                mem[i] = mem[i] * (1.0 - spike[i])
                
                # 4. DECAY: Apply decay AFTER reset (key difference!)
                mem[i] = mem[i] * self.decay
...
class Event_SMLP_Quantized(nn.Module):
    """
    Event-driven SMLP with membrane quantization support.
    NOW WITH PROPER BUFFER MANAGEMENT AND INSTANCE-LEVEL DECAY
    """
    ...
    def __init__(self, 
                ...
                mem_overflow: str = "saturate",
                track_extrema: bool = False,
                decay: float = 0.2):  # ✅ ADD DECAY PARAMETER
        super().__init__()
        
        self.layer_sizes = layer_sizes
        self.num_layers = len(layer_sizes) - 1
        self.decay = decay  # ✅ STORE INSTANCE-LEVEL DECAY
        ...
        # ✅ FIXED: Use buffers instead of ParameterList for state variables
        # Buffers are tensors that are part of the module state but don't need gradients
        for i in range(self.num_layers):
            self.register_buffer(f'mem_{i}', torch.zeros(layer_sizes[i+1]))
    ...
    def process_time_step_fast(self):
        ...
        Fast event-driven processing with optional quantization.
        No extrema tracking.
        TRAINING-SAFE: Avoids in-place operations on tensors in computation graph.
        ...
        for layer_idx in range(self.num_layers):
            ...
            mem = getattr(self, f'mem_{layer_idx}')
            fc = self.layers[layer_idx]
            
            # 1. Apply decay (create new tensor to avoid in-place on computation graph)
            mem = mem * self.decay
            
            # 2. Event-driven accumulation (only spiking inputs)
            for idx in torch.nonzero(layer_input, as_tuple=False).flatten():
                mem = mem + fc.weight[:, idx]  # Non-in-place addition
            
            # 3. QUANTIZE membrane potential (if enabled)
            if self.quant_mem:
                mem = self._quantize_if_enabled(mem)
            
            # 4. Generate spikes based on (quantized) membrane
            h_spiked = act_fun(mem)
            
            # 5. Reset spiking neurons (create new tensor, not in-place)
            # This is key: we create a new tensor instead of modifying in-place
            mem = mem * (1.0 - h_spiked)
            
            # 6. Update buffer state (detach to break gradient tape for state storage)
            getattr(self, f'mem_{layer_idx}').copy_(mem.detach())
    ...
    def process_time_step_collect_extrema(self):
        ...
        for layer_idx in range(self.num_layers):
            ...
            # 1. Apply decay (create new tensor)
            mem = mem * self.decay
            ...
            # 6. Update buffer state (detach to break gradient tape)
            getattr(self, f'mem_{layer_idx}').copy_(mem.detach())
